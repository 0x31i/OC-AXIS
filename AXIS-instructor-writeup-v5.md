...
1. Network scanning with nmap for comprehensive port and service discovery
2. Service enumeration across SSH, HTTP, RTSP, MQTT, UPnP, and ONVIF
3. Initial SSH access using default credentials (demonstrates IoT-01)
4. Filesystem mapping to identify writable directories and vendor-specific locations
5. Resource constraint assessment (memory, storage, CPU limitations)

### Phase 2: Easy Flags
1. Configuration file enumeration in vendor-specific directories (/var/lib/axis)
2. System log analysis for service banners and device identifiers
3. Web interface source code inspection for debug tokens and comments
4. Recording cache metadata analysis for privacy and analytics data
5. Factory configuration discovery revealing default credentials and backdoors

### Phase 3: Medium Flags
1. Persistent storage enumeration for backup scripts with hard-coded credentials
2. Encoded credential files requiring ROT13 decoding
3. Runtime service configuration analysis in /run directory
4. Custom script examination revealing cloud API keys and integration tokens
5. CGroup configuration for resource management and monitoring endpoints
6. UPnP device discovery and information disclosure
7. CGI script vulnerabilities including command injection and path traversal
8. SSL/TLS certificate analysis for embedded keys and identifiers
9. SUID binary enumeration in non-standard locations
10. Database file examination for application secrets

### Phase 4: Hard Flags
1. Shared memory inspection in /dev/shm for inter-process communication data
2. Binary firmware analysis using strings and hexdump techniques
3. Hidden configuration discovery in obscure filesystem locations
4. Bootloader manipulation through U-Boot environment variables
5. Hardware debug interface analysis (JTAG configuration)
6. Server-side request forgery via webhook exploitation
7. Cryptographic weakness identification and BASE64 decoding
8. Temporary cache file discovery with session tokens
9. Race condition exploitation in time-sensitive operations

---

## Key Takeaways and Defense

### OWASP IoT Top 10 Vulnerabilities Found

**IoT-01: Weak, Guessable, or Hardcoded Passwords**
- Default SSH credentials (root:pass) enabled system-wide access
- Hardcoded maintenance backdoor (axis_support account)
- Service account credentials in configuration files
- Predictable password patterns (year inclusion, dictionary words)
- Factory configuration contains all default credentials
- No password complexity requirements
- No forced password change on first boot

**IoT-02: Insecure Network Services**
- SSH banner leaking device identifiers
- UPnP without authentication exposing device information
- RTSP streams with authentication disabled
- MQTT broker with weak credentials
- ONVIF service with predictable credentials
- Debug interfaces exposed on network
- Multiple unnecessary services enabled by default

**IoT-03: Insecure Ecosystem Interfaces**
- Command injection vulnerabilities in CGI scripts
- Path traversal allowing filesystem access
- SSRF via webhook functionality
- HTML comments containing debug tokens and credentials
- API endpoints without proper input validation
- Cloud integration exposing API keys
- Management interfaces without rate limiting

**IoT-04: Lack of Secure Update Mechanism**
- Firmware signatures accessible but not verified
- No signature verification during update process
- Bootloader unlocked allowing unsigned firmware
- Update server credentials in plain text
- No rollback mechanism for failed updates
- Update endpoints without authentication

**IoT-05: Use of Insecure or Outdated Components**
- ROT13 used instead of proper encryption
- BASE64 encoding for sensitive data storage
- Deprecated cryptographic libraries (MD5, SHA1, DES references)
- Legacy protocols (FTP) for backup operations
- Outdated BusyBox version with known vulnerabilities
- No component version management

**IoT-06: Insufficient Privacy Protection**
- Recording metadata exposing personally identifiable information
- People counting and tracking data accessible
- License plate recognition data stored unencrypted
- Stream metadata revealing monitoring patterns
- Analytics data sent to cloud without encryption
- No data minimization practices

**IoT-07: Insecure Data Transfer and Storage**
- Plain text credentials in multiple configuration files
- Unencrypted certificates and private keys
- Database files world-readable containing sensitive data
- Session tokens stored in cache files
- API keys in scripts and configuration
- FTP used for backups (unencrypted protocol)
- MQTT without TLS encryption

**IoT-08: Lack of Device Management**
- Runtime services without authentication
- CGroup misconfigurations allowing DoS
- SUID binaries in custom locations
- Backup scripts with hard-coded API keys
- No centralized configuration management
- Insufficient logging and monitoring
- No resource usage limits enforced

**IoT-09: Insecure Default Settings**
- Default credentials enabled and documented
- Factory configurations accessible
- Debug mode enabled in production
- SNMP with community string "public"
- All services enabled by default
- HTTPS disabled in favor of HTTP
- Secure boot disabled

**IoT-10: Lack of Physical Hardening**
- U-Boot unlocked (secure_boot=disabled)
- JTAG debug interface accessible
- Bootloader environment readable and writable
- Hardware debug keys exposed in configuration
- No tamper detection mechanisms
- Flash storage accessible without authentication

### Defense Recommendations

#### Immediate Actions
1. Change all default credentials immediately
2. Disable unnecessary services (UPnP, SNMP, debug interfaces)
3. Remove debug configurations from production builds
4. Enable secure boot in bootloader
5. Lock JTAG and hardware debug interfaces
6. Enable HTTPS and disable HTTP
7. Remove or disable backdoor accounts

#### Configuration Hardening
1. Implement restrictive file permissions (principle of least privilege)
2. Encrypt sensitive configuration files at rest
3. Remove commented credentials and debug information
4. Disable path traversal in CGI scripts through input validation
5. Validate and whitelist webhook destinations
6. Implement proper session management with secure tokens
7. Use unique credentials per device

#### Development Practices
1. Remove all debug code and comments before production deployment
2. Implement comprehensive input validation on all endpoints
3. Use strong cryptography (AES-256, SHA-256+) instead of encoding
4. Store credentials in secure vaults, never in code or configs
...
1. Monitor shared memory for unusual access patterns
2. Log all authentication attempts with rate limiting
3. Alert on firmware modification attempts
4. Track CGI script usage and anomalies
5. Monitor for race condition exploitation indicators
6. Implement centralized logging to external SIEM
7. Regular security audits and penetration testing

#### Network Security
1. Implement network segmentation for IoT devices
2. Use VLAN isolation for camera networks
3. Enable firewall rules with default-deny policy
4. Implement SSRF protection and URL validation
5. Use VPN for remote access, not direct exposure
6. Rate limiting on all external-facing services
7. Intrusion detection and prevention systems

### Learning Objectives Achieved

**Comprehensive Filesystem Enumeration**
- Students learn to systematically explore 8 writable directory locations
- Deep directory navigation requiring 3-5 levels of exploration
- Hidden file discovery techniques and pattern recognition
- Understanding of vendor-specific directory structures

**IoT-Specific Attack Vectors**
- Recognition of embedded Linux constraints and resource limitations
- Camera-specific vulnerabilities and attack surfaces
- Firmware and bootloader exploitation techniques
- Understanding of IoT-specific protocols (RTSP, MQTT, ONVIF)
- Shared memory exploitation unique to embedded systems

**Web Application Security in IoT Context**
- Command injection in CGI scripts without framework protections
- Path traversal exploitation in lightweight web servers
- SSRF attack methodology via webhook endpoints
- Recognition of minimal input validation in embedded web interfaces

**Binary and Firmware Analysis**
- Using strings command effectively on compiled binaries
- Hexdump analysis for firmware signatures
- Bootloader configuration manipulation
- Understanding cross-compiled architectures

**Cryptographic Analysis and Encoding Recognition**
- Distinguishing encoding from encryption
- ROT13 identification and decoding
- BASE64 encoding recognition and decoding
- Weak cryptography identification in IoT context

**Advanced Exploitation Techniques**
- Race condition timing and exploitation
- Shared memory inspection and inter-process communication
- SSRF to internal services
- Resource manipulation via cgroups

**Professional Documentation and Methodology**
- Systematic enumeration methodology
- Proper tool selection for IoT environments
- Attack chain documentation
- Reporting findings with remediation recommendations

---

## Conclusion

This CTF provides comprehensive hands-on experience with IoT camera security, covering all OWASP IoT Top 10 vulnerabilities across 27 challenges. Students learn realistic exploitation techniques applicable to real-world embedded systems, with emphasis on systematic enumeration, understanding resource constraints, and recognizing the fundamental differences between IoT and traditional IT security.

**Key Success Factors**:
1. Methodical filesystem enumeration across all 8 writable directories
2. Reading file contents completely rather than searching for patterns
3. Using appropriate tools for IoT-specific challenges
4. Understanding encoding vs encryption distinction
5. Recognizing vendor-specific directory structures
6. Patience with race conditions and timing attacks
7. Comprehensive analysis of each discovered file

**Critical Differences from Traditional Pentesting**:
- Resource constraints require different exploitation approaches
- Vendor-specific directory structures instead of standard FHS
- Minimal logging requiring careful analysis of what IS logged
- Limited tooling on target requiring creative solutions
- Encoding commonly used instead of proper encryption
- Services running as root without privilege separation

**Next Steps for Advanced Learning**:
1. Practice on real AXIS camera hardware in controlled environments
2. Study AXIS VAPIX API documentation for deeper understanding
3. Learn firmware reverse engineering with binwalk and other tools
4. Understand JTAG exploitation techniques and hardware hacking
5. Develop custom exploit code for discovered vulnerabilities
6. Study other IoT vendors and platforms for pattern recognition
7. Research IoT malware (Mirai, Bashlite) for defensive understanding

**Pedagogical Notes for Instructors**:
- Emphasize systematic methodology over flag hunting
- Focus on understanding "why" vulnerabilities exist in IoT
- Connect each finding to real-world security implications
- Teach defensive thinking alongside offensive techniques
- Use this CTF as foundation for broader IoT security curriculum
- Encourage students to document their methodology
- Discuss ethical considerations and responsible disclosure

---

**Educational Use Only**: All techniques in this writeup are for authorized security testing and education only. Unauthorized access to systems is illegal. This material should only be used in controlled lab environments or with explicit written authorization from system owners.
